...
const FILM_GRID_SKELETON_KEYS = Array.from({ length: 24 }, (_, i) => `film-skeleton-${i}`);

function FilmGridSkeleton() {
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 2xl:grid-cols-7 gap-4">
      {FILM_GRID_SKELETON_KEYS.map(key => (
        <div key={key} className="rounded-lg bg-card border border-border overflow-hidden">
