...
This is a deepsec scanning workspace. Project `api` was just registered
(target: `../apps/api`). Setup is incomplete — `data/api/INFO.md`
...
2. **Fill in `data/api/INFO.md`.** It's auto-injected into the AI
   ...
   **Per-section rubric**:
   ...
   - `../apps/api/README.md`
   - any `AGENTS.md` / `CLAUDE.md` in `../apps/api`
   - `../apps/api/package.json` (or `go.mod`, `pyproject.toml`, etc.)
...
```bash
pnpm deepsec scan    --project-id api
pnpm deepsec process --project-id api
```
